...
/*----------------------------------------Registration API----------------------------------------------*/	
	...
	function register_post(){
		...
		{
			$response= array('status'=>'201', 'message'=>'profile_pic missing!', 'data'=>'');
		}
...
/*----------------------------------------Login API----------------------------------------------*/	
	...
	function update_profile_post(){		
		...
		$otp_no = "123456";//$otp_n;			
			...
			{
				$u_data = $this->wb_model->getsingle('ac_users',array('user_id'=>$user_id));
				...
				$this->load->library('upload', $data);			
				$user_profile_pic= $u_data->user_profile_pic;
				if ($this->upload->do_upload('profile_pic'))
				{
					$attachment_data = array('upload_data' => $this->upload->data());
					$user_profile_pic = $attachment_data['upload_data']['file_name'];
...
/*----------------------------------------Category API----------------------------------------------*/	
	...
	function categories_get(){ 		
		...
		if(count($data)>0)
		{
			...
			foreach($data as $d)
			{
				$current_product 	= $this->wb_model->getAllwhere('ac_products',array('category_id' => $d->category_id,'bid_start_date_time <=' => date('Y-m-d h:i:s'),'bid_end_date_time >=' => date('Y-m-d h:i:s')));
				$past_product 		= $this->wb_model->getAllwhere('ac_products',array('category_id' => $d->category_id,'bid_start_date_time <' => date('Y-m-d h:i:s'),'bid_end_date_time <' => date('Y-m-d h:i:s')));
				$future_product 	= $this->wb_model->getAllwhere('ac_products',array('category_id' => $d->category_id,'bid_start_date_time >' => date('Y-m-d h:i:s'),'bid_end_date_time >' => date('Y-m-d h:i:s')));
				$final_data['category_id'] 		= $d->category_id;
				$final_data['name'] 			= $d->name;
				$final_data['current_product'] 	= count($current_product);
				$final_data['past_product'] 	= count($past_product);
				$final_data['future_product'] 	= count($future_product);
				if($d->icon!='')
				{
	...
	function products_get(){       
		...
		if(count($data)>0)
		{
			...
			foreach($data as $d)
			{
				...
				if($d->bid_end_date_time>date('Y-m-d h:i:s'))
				{	
					$endt = strtotime($d->bid_end_date_time);
					$crtime = strtotime(date('Y-m-d h:i:s'));			
					$final_time =  round(abs($endt - $crtime) / 60);
					if($final_time>0)
					{
						$final['end_min'] 	= $final_time;
					}else{
						$final['end_min'] 	= "0";
					}
				}else{
	...
	function products_by_category_post(){
		...
		if(count($data)>0 && $category_id!='')
		{
			...
			foreach($data as $d)
			{
				...
				$final['bid_start_date_time'] 	= $d->bid_start_date_time;
				$final['bid_end_date_time'] 	= $d->bid_end_date_time;
				$watch_list = $this->wb_model->getsingle('ac_watch_list',array('user_id' => $user_id,'product_id' => $d->product_id));		
				if($watch_list)
				{
				$final['watch_list_status'] 	= "1";	
				}
				else
				{
				$final['watch_list_status'] 	= "0";		
				}
				//documents
				$documents = $this->wb_model->getAllwhere('ac_product_documents',array('product_id' => $d->product_id));
				if(count($documents)>0)
...
/*----------------------------------------Wallet API----------------------------------------------*/		
	...
	function place_bid_post(){
			...
			if($user_id!='' && $users_data && $product_id!='' && $products_data  && $products_data->bid_end_date_time >= date('Y-m-d h:i:s') && $products_data->current_bid_amount < $bid_amount)
			{
				...
				// If Bid is last 10 minit before then update 20minit + of bid end date time
				$endt = strtotime($products_data->bid_end_date_time);
				$crtime = strtotime(date('Y-m-d h:i:s'));			
				$final_time =  round(abs($endt - $crtime) / 60);
				if($final_time<=10)
				{
					$currentDatetime = strtotime($products_data->bid_end_date_time);
					$futureDatetime = $currentDatetime+(60*20);		
					$extratime =  date("Y-m-d h:i:s",$futureDatetime);
					$this->wb_model->updateData('ac_products',array('bid_end_date_time'=> $extratime),array('product_id'=> $product_id));
				}
